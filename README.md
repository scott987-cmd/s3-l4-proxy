...
flowchart LR
    ...
    C[L4 CLB<br>TCP 443 passthrough<br>ACL allowlist]
...
# edit config.env: S3_BACKEND_HOST, S3_BACKEND_PORT, S3_CLIENT_HOST, S3_REGION, LISTEN_PORT, CLB_IP
sudo bash scripts/install_l4_proxy.sh CONFIG=config.env
bash scripts/verify_l4_proxy.sh -c config.env
```

From a client machine that can reach the CLB VIP/EIP:

```bash
S3_ACCESS_KEY=... S3_SECRET_KEY=... CLB_IP=<clb-ip> \
S3_CLIENT_HOST=<signed-host> S3_REGION=<region> \
  bash scripts/speed_test_l4.sh
```

The test keeps `S3_CLIENT_HOST` as both the TLS SNI and the signed host, and uses curl `--resolve` only to force TCP to the CLB — certificate validation and host-signing semantics are preserved.
...
# Accept — add RUN_SPEED=1 for a real PUT/GET/MD5 through the CLB
...
| `scripts/speed_test_l4.sh` | S3 PUT/GET/DELETE throughput and integrity test through the CLB. |
| `scripts/diag_clb_l4.sh` | CLB/backend connectivity diagnosis, no credentials required. |
...
2. The client uses `S3_CLIENT_HOST` for TLS SNI and request signing, and DNS (public, private, or controlled resolution) routes that hostname to the CLB.
...
5. The CLB listener is TCP (layer 4); no device in the path modifies TLS or the application-layer request.
...
| Transport | End-to-end TLS | Neither CLB nor nginx terminates TLS; no certificates or private keys on the proxy. |
| Ingress | CLB ACL + security group | Allow only the client's fixed egress IPs; ECS 443 accepts only CLB/approved sources. |
...
`speed_test_l4.sh` uses curl SigV4 and therefore suits AWS-SigV4-compatible endpoints. For vendor-specific authentication, use the vendor CLI/SDK while keeping the same CLB hostname resolution path.

## Observability

| Dimension | Signal | Suggested alert |
| --- | --- | --- |
| Liveness | nginx, 443 listener, healthy CLB backends | Any node down, or healthy backends below N+1 |
...
- [ ] At least two ECS nodes in different availability zones behind the same L4 CLB
- [ ] CLB TCP health check targets the same `LISTEN_PORT` as business traffic
- [ ] CLB ACL and ECS security group allow inbound 443 only from the client's fixed egress / the CLB
...
- [ ] `ENABLE_PROXY_PROTOCOL` matches the CLB listener on both sides
...
- [ ] Real S3 PUT/GET/MD5 through the CLB passes before cutover
...
- The proxy is stateless; run at least two nodes behind a CLB for production.
