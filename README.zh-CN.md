...
| 需求 | 四层 TCP 透传 | 七层网关 |
| --- | --- | --- |
| TLS 与签名原样透传 | ✅ 推荐 | 需严格保持签名语义 |
| 虚拟 AK/SK、代签或重签 | ❌ | ✅ |
| Host/path 改写、寻址模式转换 | ❌ | ✅ |
| 对象级鉴权、限流、审计 | ❌（HTTP 不可见） | ✅ |
| WAF Header/Body 检查 | ❌ | ✅ |

需要一条透明网络通路时选四层；需要**理解或改写** S3 语义时选七层。
...
- **四层可见性限制。** 代理看不到加密后的 HTTP/S3 语义，无法记录 bucket、object key、access key 或请求动作。`nginx stream` 只能记录来源、时长、字节数、状态和上游地址。对象级审计必须由客户端、对象存储审计日志或七层网关完成。
