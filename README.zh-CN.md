...
**人力：部署一条命令，日常一条命令。** 部署、巡检、验收、运维各一条命令，`health` 的退出码 0/1/2 可直接接入监控告警。证书轮换零动作、密钥轮换零动作、后端 DNS 变更由定时器自动跟进、扩容只是把同一份配置装到新节点再加进 CLB——没有需要人盯着的周期性操作。

**成本：全开源，无授权费用。** 数据面只依赖发行版官方仓库的 nginx 与 `ngx_stream_module`：无闭源组件、无 license 费用、无按量计费的中间件、无厂商锁定。全部资源开销就是两台 ECS 与一个四层 CLB；不解密不解析，CPU 花在网络 I/O 上，普通规格实例即可承载。
...
# 填写 S3_BACKEND_HOST、S3_BACKEND_PORT、S3_CLIENT_HOST、S3_REGION、LISTEN_PORT、CLB_IP
sudo bash scripts/install_l4_proxy.sh CONFIG=config.env
bash scripts/verify_l4_proxy.sh -c config.env
```

在能访问 CLB VIP/EIP 的客户端机器上：
...
测试把 `S3_CLIENT_HOST` 同时作为 TLS SNI 和签名 host，只用 curl `--resolve` 把 TCP 强制指向 CLB——证书校验与 host 签名语义都得以保留。
...
# 验收——加 RUN_SPEED=1 会经 CLB 做真实 PUT/GET/MD5
...
| `scripts/speed_test_l4.sh` | 经 CLB 的 S3 PUT/GET/DELETE 吞吐与完整性测试。 |
| `scripts/diag_clb_l4.sh` | 免凭证的 CLB/后端连通性诊断。 |
...
- 环境无法提供 CLB ACL、安全组白名单或多后端高可用（裸露单台 ECS 公网 IP 不是可接受的替代）。
...
| 传输层 | 端到端 TLS | CLB 与 nginx 都不终结 TLS，代理机不存证书和私钥。 |
...
- **PROXY protocol 默认关闭。** 只有 CLB 监听明确启用时才设 `ENABLE_PROXY_PROTOCOL=1`——两端不一致会让普通 TLS 客户端全部连不上。
...
这里说的兼容是 **TCP/TLS 数据面**兼容，不代表不同厂商的签名协议天然互通。上线前必须用客户真实 SDK/CLI 验证 PUT、GET、HEAD、Multipart 与完整性。
...
`speed_test_l4.sh` 用 curl SigV4，适用于兼容 AWS SigV4 的 endpoint。厂商特有认证请改用厂商 CLI/SDK，但保持同样的 CLB hostname 解析路径。
...
- [ ] CLB TCP 健康检查指向与业务相同的 `LISTEN_PORT`
- [ ] CLB ACL 与 ECS 安全组只放行客户端固定出口 / CLB 的 443 入向
