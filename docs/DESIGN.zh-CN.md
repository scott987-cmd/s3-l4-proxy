...
| 需求 | 四层 TCP 透传 | 七层网关 |
| --- | --- | --- |
| TLS 与签名原样透传 | 推荐 | 需要严格保持签名语义 |
| 虚拟 AK/SK、代签或重签 | 不支持 | 支持 |
| Host/path 改写、寻址转换 | 不支持 | 支持 |
| 对象级鉴权、限流和审计 | 不可见 HTTP 语义 | 支持 |
| WAF Header/Body 检查 | 不支持 | 支持 |
...
> ❗ L4 代理看不到加密后的 HTTP/S3 语义，不能记录 bucket、object key、access key 或请求动作。nginx stream 只能记录连接来源、连接时长、字节数、状态和上游地址。对象级审计必须由客户端、对象存储审计日志或七层网关完成。
